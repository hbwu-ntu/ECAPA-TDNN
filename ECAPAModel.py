...
class ECAPAModel(nn.Module):
    def __init__(self, lr, lr_decay, C, n_class, m, s, test_step, **kwargs):
        ...
        print(
            ...
            % (
                sum(param.numel() for param in self.speaker_encoder.parameters())
                / 1024
                / 1024
            )
        )
    ...
    def speaker_similarity(self, eval_list):
        self.eval()
        ...
        for line in tqdm.tqdm(lines, total=len(lines)):
            ...
            audio, _ = soundfile.read(path1)
            data_1 = torch.FloatTensor(numpy.stack([audio], axis=0)).cuda()
            audio, _ = soundfile.read(path2)
            data_2 = torch.FloatTensor(numpy.stack([audio], axis=0)).cuda()
